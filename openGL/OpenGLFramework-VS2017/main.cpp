...
GLuint iLocView, iLocKa, iLocKd, iLocKs, iLocLightmode, iLocLighta, iLocLightd, iLocLights, iLocPLpos;
int light_mode = 0;
float shininess = 64.0f;
Vector3 PL_position = Vector3(0, 2, 1);
...
void RenderScene(int per_vertex_or_per_pixel) {	
	...
	for (int i = 0; i < models[cur_idx].shapes.size(); i++)
	{
		...
		if (keynum.near_line == 0 && keynum.near_mmline == 0) {
			glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
			glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
		}
		else if (keynum.near_mmline == 1) {
			keynum.near_line = 0;
			glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_LINEAR);
			glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR_MIPMAP_LINEAR);
		}
		else if(keynum.near_line == 1){
			keynum.near_mmline = 0;
	...
	glUniform3f(iLocView, main_camera.position.x, main_camera.position.y, main_camera.position.z);
	for (int i = 0; i < models[cur_idx].shapes.size(); i++) {
	...
	glUniform3f(iLocLighta, 0.15f, 0.15f, 0.15f);
	glUniform3f(iLocLightd, 1.0f, 1.0f, 1.0f);
	glUniform1f(iLocLights, shininess);
	glUniform3f(iLocPLpos, PL_position.x, PL_position.y, PL_position.z);
}
...
void KeyCallback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	if (action == GLFW_PRESS) {
		...
		case GLFW_KEY_G:
			...
			break;
		case GLFW_KEY_B:
			keynum.near_mmline += 1;
			keynum.near_mmline %= 2;
			break;
		case GLFW_KEY_L:
			light_mode += 1;
...
GLuint LoadTextureImage(string image_path)
{
	...
	if (data != NULL)
	{
		...
		glTexImage2D(GL_TEXTURE_2D, 0, GL_BGRA, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, data);
...
void setUniformVariables()
{
	...
	iLocView = glGetUniformLocation(program, "viewPos");
	iLocKa = glGetUniformLocation(program, "materialKa");
	iLocKd = glGetUniformLocation(program, "materialKd");
	iLocKs = glGetUniformLocation(program, "materialKs");
	iLocLightmode = glGetUniformLocation(program, "light_mode");
	iLocLighta = glGetUniformLocation(program, "light_ambient");
	iLocLightd = glGetUniformLocation(program, "light_diffuse");
	iLocLights = glGetUniformLocation(program, "Shininess");
	glUniform3f(glGetUniformLocation(program, "dirLight.position"), 1.0f, 1.0f, 1.0f);
	glUniform3f(glGetUniformLocation(program, "dirLight.direction"), -1.0f, -1.0f, -1.0f);
	glUniform3f(glGetUniformLocation(program, "light_specular"), 1.0f, 1.0f, 1.0f);
	iLocPLpos = glGetUniformLocation(program, "pointLight.position");
	glUniform1f(glGetUniformLocation(program, "pointLight.constant"), 0.01f);
	glUniform1f(glGetUniformLocation(program, "pointLight.linear"), 0.8f);
	glUniform1f(glGetUniformLocation(program, "pointLight.quadratic"), 0.1f);
	glUniform3f(glGetUniformLocation(program, "spotLight.position"), 0.0f, 0.0f, 2.0f);
	glUniform3f(glGetUniformLocation(program, "spotLight.direction"), 0.0f, 0.0f, -1.0f);
	glUniform1f(glGetUniformLocation(program, "spotLight.exponent"), 50.0f);
	glUniform1f(glGetUniformLocation(program, "spotLight.cutoff"), (acos(-1) * 30) / 180);
	glUniform1f(glGetUniformLocation(program, "spotLight.constant"), 0.05f);
	glUniform1f(glGetUniformLocation(program, "spotLight.linear"), 0.3f);
	glUniform1f(glGetUniformLocation(program, "spotLight.quadratic"), 0.6f);

	// [TODO] Get uniform location of texture
	iLocTex = glGetUniformLocation(program, "umtex");
}
...
int main(int argc, char **argv)
{
    ...
    // create window
	GLFWwindow* window = glfwCreateWindow(WINDOW_WIDTH, WINDOW_HEIGHT, "106062122 HW3", NULL, NULL);
    if (window == NULL)
    {
